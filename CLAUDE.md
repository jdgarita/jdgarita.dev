...
A plain static personal site for `jdgarita.dev`, served by **Cloudflare Workers Static Assets** (Worker `jdgarita-dev`, config in `wrangler.jsonc`) straight from the repo root — no build. `index.html` is the entry point; everything is hand-written vanilla HTML/CSS/JS. The only third-party CSS loaded is Google Fonts (Inter + JetBrains Mono). Icons are an inline SVG sprite (Lucide + Simple Icons); there is no icon library or icon font. There is no Bootstrap, no jQuery, no build step, no package manager, and no lint/test/build CI (the only Actions are the Claude Code review workflows listed below). The one server-side piece is `worker.js`, which handles only `POST /e` (the analytics beacon); its tests run with Node's built-in runner (`scripts/worker.test.mjs`, no dependencies). Edits to the source files are the deliverable — open `index.html` in a browser to preview.
...
- `main` is the default branch and the only long-lived one. It is PR-protected — land changes via short-lived feature branches (e.g. `fix/…`, `docs/…`, `content/…`) merged through a PR, not by pushing directly to `main`. Merging to `main` is what deploys (Cloudflare Workers Builds, connected to this repo); every other branch gets a Workers preview URL, whose beacons are never relayed.
...
- **Preview locally with HTTP** (recommended): `python3 -m http.server 8000` from the repo root, then open `http://localhost:8000`. This is enough for pages, i18n, and styling (`fetch('i18n/en.json')` succeeds); the analytics beacon's `POST /e` just fails silently.
- **Full production parity** (Worker + `/e` + 404 handling): `npx wrangler dev` from the repo root (`http://localhost:8787`). Beacons there answer 204 but are never relayed — only `jdgarita.dev` relays to PostHog.
...
- **No third-party JS; first-party analytics on the landing only.** The legal pages' only script is the one-line year stamp (no analytics). The landing's only script is the same self-contained inline block (year stamp + EN/ES i18n + light/dark theme toggle, persisted to `localStorage` keys `still-lang` / `still-theme`, + the analytics beacon: a `pageview` and a `store_click` per click on an `a[data-store]` with `data-placement`, sent to `/e` — see "Analytics"); there are **no external/CDN dependencies**. A new store link needs `data-store` + `data-placement` or its clicks go uncounted. The Still privacy policy's "Website Analytics" section discloses this; keep it true when changing what is sent. The landing's **theme toggle** sets `data-theme` on `<html>` (a no-FOUC head script applies the stored/OS choice before paint); the **legal pages have no toggle and follow the OS** via `prefers-color-scheme`. The phone mock-ups are pure static HTML/CSS (inline SVG sprite — Lucide line icons + the app's food glyphs), so the page still renders with JS disabled (English, OS theme).
...
First-party, cookieless, and relayed server-side to **PostHog** — the same design as faint.coffee's landing. PostHog project: the **frnk** project on PostHog Cloud US (shared with faint.coffee and the apps; filter on `site = jdgarita.dev`).

- **Client:** `logEvent(name, params)` in `js/custom.js` (root, `frnk/`, `404.html`) `sendBeacon`s a small JSON payload to `/e` on this site. The Still landing has its own inline copy (`pageview` + `store_click`). No third-party script, no cookie. The only identifier is a random id in `sessionStorage` (`jd_vid`, one tab); the visit's referrer + UTM tags are kept in `jd_attr` so later events in the tab keep the attribution. Fire-and-forget: it never blocks a click, and without JS the site works unchanged.
- **Server:** `worker.js` (only `POST /e` reaches it) refuses cross-origin beacons, drops bots and bodies over 2 KB, keeps only allowlisted events + properties (the `EVENTS` map), strips query strings from URLs, and relays to PostHog's capture API from `ctx.waitUntil`. The visitor's IP never reaches PostHog. Only requests to `jdgarita.dev` relay; `wrangler dev`, previews, and `workers.dev` accept and drop.
- **Key:** the project's public `phc_…` key is a Worker secret, never committed: `npx wrangler secret put POSTHOG_API_KEY`. Failed captures are logged to Workers Logs (`observability` in `wrangler.jsonc`).
- **Events:** `$pageview` (all pages; `not_found: true` on 404s), `file_download` (resume; `link_location` nav/hero), `select_content` (`content_type` `nav_section` / `contact_link` / `project_link`, with a slug `content_id`), `store_click` (Still landing; `platform`, `placement`), `theme_toggle`, `lang_toggle`, `mobile_nav_toggle`. Every event carries `site`, `$current_url`, `$pathname`, `$referrer`, `lang`, `country`, and UTM tags when present.
- **Adding an event or property:** send it from the page **and** add it to `EVENTS` in `worker.js` (plus a test in `scripts/worker.test.mjs`) — anything not in the allowlist is refused or dropped.
